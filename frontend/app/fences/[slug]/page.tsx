...
export async function generateStaticParams() {
  return fences.map((fence) => ({ slug: fence.slug }));
}
...
export default async function FenceDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  // Related = same material, excluding current
  const related = fences
    .filter((f) => f.material === fence.material && f.id !== fence.id)
    .slice(0, 3);
